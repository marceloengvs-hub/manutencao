...
export default function Login() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      result = await signUpWithEmail(email, password, nome)
    }
  ...
  return (
    <div
      ...
      style={{ background: 'var(--color-surface-main)' }}
    >
      {/* Ambient glow */}
      <div
      ...
      >
        {/* Top accent bar */}
        <div style={{ height: '2px', background: 'var(--color-accent)' }} />
        ...
        <div className="px-8 py-10 flex flex-col items-center">
          ...
          <p className="text-xs mt-6 text-center" style={{ color: 'var(--color-text-muted)' }}>
            Acesso restrito a membros autorizados do laboratório.
          </p>
        </div>
      </div>
    </div>
